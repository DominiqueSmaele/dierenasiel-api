...
@if ($paginator->hasPages())
    <nav class="mb-3 mt-12 flex flex-wrap justify-start">
        <div class="flex items-center">
            <x-button class="flex" type="button" variant="primary" color="orange" :disabled="$paginator->onFirstPage()" wire:click="previousPage('{{ $paginator->getPageName() }}')" rel="prev" aria-label="{{ __('pagination.previous') }}">
                <x-icon.arrow-left class="h-4 w-4" />
            </x-button>

            <x-button class="ml-2 flex" type="button" variant="primary" color="orange" :disabled="!$paginator->hasMorePages()" wire:click="nextPage('{{ $paginator->getPageName() }}')" rel="next" aria-label="{{ __('pagination.next') }}">
            ...
            @if ($translationKey)
                ...
                <div class="ml-3 flex rounded-full bg-blue-base px-3 py-1 text-sm text-white">
